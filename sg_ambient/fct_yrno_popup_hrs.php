<?php $scrpt_vrsn_dt  = 'fct_yrno_popup_hrs.php|00|2020-04-06|';  # beta
#-----------------------------------------------
...
$cl_headers     = array ('from' => lang('Period'), 
...
#                         'uvuv' => lang('UV index'), 
                         'baro' => lang('Pressure')  );
$cl_cntnt       = array ('from','icon','temp', 'dewp', 'feel', 'humi', 'uvuv', 'r_ch', 'rain',
...
for ($n = 0; $n < $rows; $n++) // print 1 row / daypart with all data in coloms
     {  $arr    = $parts[$n];   echo  '<!-- '.print_r($arr,true) .' -->';  
        ...
        if ($ymd <> $ymd_old)
             {  $sunrise = date_sunrise($arr['unix'], SUNFUNCS_RET_TIMESTAMP, $lat, $lon);
                ...
                $set_hr= date('H',$sunset);
  #              if ($set_hr == '00') {$set_hr = 24;}
...
<img src="img/sunset.png" alt="sunset">&nbsp;&nbsp;'.$set.'&nbsp;&nbsp;&nbsp;'.lang ('Daylight') .': '.$length.'&nbsp;</span></td></tr>'.PHP_EOL.$head_str;	        
        ...
        if ((int) $arr['to'] == 00) {$to = 24;}  else {$to = $arr['to'];}
        if ($arr['from'] > $rise_hr && $to <= $set_hr) {$colorx = $clrwrm; } else {$colorx = $clrcld;}
        foreach ($cl_headers as $key => $header)  // for every data value = column we want to print
            {   if (isset ($arr[$key]) ) 
                ...
                switch ($key){
                    case 'from': 
                        echo PHP_EOL.'<td><span style="font-size: 14px; color: '.$colorx.';">'.$arr['from'].' -  '.$arr['to'].'</span>'; 
                        break;
                    case 'icon':
